...
if a.shape[0] == 0 :
    ...
    for i in range(npart):
        ...
        for j in range(n_levels-1):
            ...
            loading_i = loading2D[:,:,column + j + 1]
            loading2D_sum += loading_i

            print 'Class CL',str(i+1).zfill(2),', from level '+str(H_LEVELS[j,0])+' to level '+str(H_LEVELS[j+1,0])+' mass ','%.1e'%mass_in_the_air,' kg'

            

            # Save the deposit for the single classes on a ESRI rater ascii file
            output_file = runname+'_'+'CL'+str(i+1)+'_H_'+str(H_LEVELS[j+1,0])+'_'+day+'_'+time+'.asc'
            ...
            plt.grid()
            plt.title('Class CL'+str(i+1).zfill(2)+' Height '+str(H_LEVELS[j+1,0])+' mass '+'%.1e'%mass_in_the_air+' kg' )
            clb = plt.colorbar(format=ticker.FuncFormatter(fmt))
            clb.set_label('Loading (kg/m^3)', labelpad=-40, y=1.05, rotation=0)

            f.savefig(runname+'_'+'CL'+str(i+1)+'_H_'+str(H_LEVELS[j+1,0])+'_'+day+'_'+time+'.pdf', bbox_inches='tight')
